...
- `combat` — depends on `fleet`, `defense`, `catalog`, `research`, `resource`, `planet`, `building`. Resolves battles (`AttackArrived`/`BombardArrived`/`InvadeArrived` → `BattleReport`/`PlanetDestroyed`/`PlanetInvaded`); the `planet`/`building` dependencies exist specifically to destroy a planet (wipe buildings/resources/fleet/defenses, mark it permanently `destroyed`) or transfer ownership on a successful invasion.
...
- **Buildings and ships** use one fixed icon per catalog `key` (no variance — the type itself is the visual identity, so `key`-based lookup is enough, no backend field needed). Assets present: `frontend/public/images/buildings/<key>.webp` for all 9 building keys. `frontend/public/images/ships/` is still empty — ship icons haven't been generated yet, so ship cards fall back to no icon via `appImgFallback` until `<key>.webp` files are added there for all 12 ship keys (see `src/main/resources/catalog/ships.json`).

### Ship roster, gating, and the BOMBARD/INVADE missions

`ships.json` has 7 combat ship tiers (`light_fighter` → `galaxy_class`, alternating tank/glass-cannon archetypes) plus 3 utility ships (`small_cargo`, `colony_ship`, `espionage_probe`) plus 2 end-game "weapon of last resort" ships (`orbital_bomb`, `invasion_unit`). Unlike buildings/technologies, ships historically had no gating at all; `ShipDefinition.requirements` (a `List<Requirement>`, same shape as `BuildingDefinition`/`TechnologyDefinition`) now gates the newer ships behind shipyard level and/or technology, computed by `FleetService.missingRequirements` (checks `BUILDING` per-planet via `BuildingService.levelOf` and `TECHNOLOGY` account-wide via `ResearchService.levelOf` — unlike `BuildingService`'s own building-only requirement check) and surfaced on `ShipyardView.unlocked`/`missingRequirements`, enforced server-side in `FleetService.queueShip` too (not just hidden client-side).

`orbital_bomb`/`invasion_unit` are gated behind two deep end-game technologies (`orbital_bombardment`, `planetary_invasion_tactics` in `technologies.json`) plus shipyard level 20, by design (see `FleetService`'s `BOMBARD`/`INVADE` mission types): they permanently destroy or capture an enemy **colony** (never a homeworld — `validateSpecialMissionTarget` rejects homeworld targets), require a `galaxy_class` escort and can't be sent together or alone (`FleetService.validateSpecialShipComposition`), and resolve in `CombatService.resolveSpecialMission` — an undefended target skips combat and goes straight to a 95% success roll; a defended target fights a normal battle first (`CombatService.resolveCombat`, shared with `ATTACK`, publishing a `BattleReport` either way) and the special ship only gets its roll if it survives. A successful destroy wipes the planet's buildings/resources/fleet/defenses via each module's `deleteAllForPlanet`/`wipeAllShipsAndOrders` and sets `Planet.destroyed = true` (a permanent tombstone — `SlotStatus.DESTROYED` in the system view, never recolonizable, `FleetService.requireNotDestroyed` rejects further missions at it); a successful invasion just reassigns `Planet.ownerId` (buildings/resources/stationed fleet transfer as-is, since they key on `planetId` not `ownerId`).
